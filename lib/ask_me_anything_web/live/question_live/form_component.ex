defmodule AskMeAnythingWeb.QuestionLive.FormComponent do
  ...
  def update(%{question: question} = assigns, socket) do
    changeset = LiveInteraction.change_question(question)
  ...
  defp save_question(socket, :edit, question_params) do
    case LiveInteraction.update_question(socket.assigns.question, question_params) do
      {:ok, _question} ->
        {:noreply,
         socket
  ...
  defp save_question(socket, :new, question_params) do
    case LiveInteraction.create_question(question_params) do
      {:ok, _question} ->
        {:noreply,
         socket
